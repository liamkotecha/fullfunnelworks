/**
 * Product: Positioning Checklist — animated checklist
 */
"use client";
...
import { PRODUCT_SECTION } from "@/lib/concept-map";
...
export default function ProductChecklistPage() {
  ...
  const { questions: dbQuestions, loading: questionsLoading } = useQuestions("product", "challenges");
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <SectionProgressHeader
        title={PRODUCT_SECTION.currentChallenges.heading}
      ...
      <div className="mt-6 bg-white rounded-lg shadow-sm ring-1 ring-slate-200 p-8 space-y-6">
        <SectionGuidance
          intro={PRODUCT_SECTION.intro}
          estimatedMinutes={10}
        />

        {allComplete && !reviewMode ? (
          <WhatsNext
            completedTitle="Product Positioning Actions"
            nextTitle="Customer Outcome Mapper"
            nextHref="/portal/product/outcomes"
            nextDescription="Feature → Problem → Outcome → Impact"
              ...
              {sortedIndices.map((i) => {
                ...
                return (
                  <AutosaveField
                    ...
                    onSaveSuccess={(result) => setLocalProgress("product-challenges", result)}
                  >
                    {({ onChange }) => (
                      <motion.div
                        layout
