/**
 * People: Challenges & Strategy — animated checklist
 */
"use client";
...
import { PEOPLE_SECTION } from "@/lib/concept-map";
import { useQuestions } from "@/hooks/useQuestions";
import { Skeleton } from "@/components/ui/Skeleton";
type Phase = "idle" | "tick" | "strike" | "done";

export default function PeopleChecklistPage() {
  ...
  const { questions: dbQuestions, loading: questionsLoading } = useQuestions("people", "challenges");
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <SectionProgressHeader
        title={PEOPLE_SECTION.currentChallenges.heading}
      ...
      <div className="mt-6 bg-white rounded-lg shadow-sm ring-1 ring-slate-200 p-8 space-y-6">
        <SectionGuidance
          intro={PEOPLE_SECTION.intro}
          estimatedMinutes={10}
        />

        {allComplete && !reviewMode ? (
          <WhatsNext
            completedTitle="People Development Actions"
            nextTitle="Team Capability Tracker"
            nextHref="/portal/people/methodology"
            nextDescription="Map team skills and training gaps"
              ...
              {sortedIndices.map((i) => {
                ...
                return (
                  <AutosaveField
                    ...
                    onSaveSuccess={(result) => setLocalProgress("people-challenges", result)}
                  >
                    {({ onChange }) => (
                      <motion.div
                        layout
